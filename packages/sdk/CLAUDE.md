...
- `deposit` / `withdraw` build an action-shaped `LeverageYieldSwapPayload` (`{ params: CreateIntentParams, hubWalletSwap?: true, partnerFee? }`) — the caller spreads it into `sodax.leverageYield.vaultSwap()`. `withdraw` sets `hubWalletSwap: true` so the vault swap spends the `lsoda*` held in the user's hub wallet via a `Connection.sendMessage` (the `createVaultIntent` hub-wallet branch validates the input token against the hub chain, not `srcChainKey`).
- `createVaultIntent` / `vaultSwap` — leverage-yield copies of the swap domain's `createIntent` / `swap()`. **Deliberate cross-domain duplication** (client review decision): the vault execution modifiers (`hubWalletSwap`, per-intent `partnerFee` beating `config.swaps.partnerFee`) live on `VaultSwapActionParams` (the leverage-yield action wrapper) so the generic swap surface stays vault-free. Don't "deduplicate" them back into `SwapService`, and don't move execution modifiers onto `CreateIntentParams` — the shared intent input type stays a pure description per the `SpokeExecActionParams` docstring.
...
- **Shares always land in the derived hub wallet, and lsoda\* is NOT recoverable.** `deposit` sets `dstAddress = getUserHubWalletAddress(srcAddress, srcChainKey)` for every source chain — including Sonic, where that's the CREATE3 user-router, not the signing EOA — and `withdraw` (`hubWalletSwap`) spends from the same address. Consumers reading share balances must resolve the holder the same way (never `balanceOf(EOA)` on the hub). Because `lsoda*` tokens sit in `sonicSupportedTokens` with `hubAsset` = the vault proxy, they would otherwise surface in `RecoveryService.fetchHubAssetBalances` as stranded assets — but they aren't asset-manager-registered, so the recovery `assetManager.transfer` reverts on them (a failed gas estimate in the wallet). `fetchHubAssetBalances` filters them out by `leverageYield.vaults[].vault` address; positions are exited via the leverage-yield `withdraw` flow, not recovery.
- **Errors**: All async methods return `Result<T, SodaxError<NarrowCode>>`. User-facing actions discriminate via `context.action` (`'deposit' | 'withdraw' | 'approve' | 'vaultSwap'`); `isAllowanceValid`'s allowance pre-flight uses `action: 'allowanceCheck'` so its failures don't masquerade as `'deposit'`; read methods emit `LOOKUP_FAILED` partitioned by `context.method`. Relay/tx-verification codes (`TX_VERIFICATION_FAILED`, `TX_SUBMIT_FAILED`, `RELAY_TIMEOUT`, `RELAY_FAILED`) appear only on `vaultSwap` (`LeverageYieldSwapError`) — `createVaultIntent` alone stays within the create-intent subset. `notifySolver` is public (the standalone solver-notify step for manual create→relay→notify orchestration) and emits the post-execution subset (`EXECUTION_FAILED` / `EXTERNAL_API_ERROR` / `UNKNOWN`), which `vaultSwap` also surfaces. See `docs/LEVERAGE_YIELD.md`.
...
- `SWAPS.md`, `SWAPS_API.md` (the `sodax.api.swaps` HTTP client), `BACKEND_API.md`, `MONEY_MARKET.md`, `STAKING.md`, `BRIDGE.md`, `DEX.md`, `MIGRATION.md`, `LEVERAGE_YIELD.md`
