...
class SodaxError<C extends SodaxErrorCode = SodaxErrorCode> extends Error {
  ...
  readonly feature: SodaxFeature;    // 'swap' | 'moneyMarket' | 'bridge' | … — defined in @sodax/types, re-exported from errors/codes.ts
...
Separate from logging: the SDK can emit **structured, opt-in user-action events** (feature action flows) to a consumer-supplied sink. Distinct from `logger` in two ways that matter — it is **off by default** (logger is on), and events are **structured** (`feature` + `action` + `phase` + `level` + `data`) rather than free-form messages.

- **Enable:** `new Sodax({ analytics: { sink, level?, features? } })`. The `analytics` option lives on `SodaxOptions` next to `logger` (same rationale — client-side runtime, never on `SodaxConfig`, never fetched/overwritten by the backend). Omit it (or pass `false`) to stay disabled.
- **Types** live in `@sodax/types` (`shared/analytics.ts`): `SodaxAnalytics` (`track(event)`), `AnalyticsEvent`, `AnalyticsConfig` (`sink` + optional `level: 'basic' | 'detailed'` + per-feature `features` map), `AnalyticsOption = AnalyticsConfig | false`. The `feature` field is `SodaxFeature` (now sourced from `@sodax/types`, re-exported from `errors/codes.ts`) so analytics events and `SodaxError`s share one feature taxonomy.
- **Resolution** lives in `shared/analytics.ts` (`resolveAnalytics`, `noopAnalytics`, `ResolvedAnalytics`, exported from the barrel). `resolveAnalytics` defaults to `noopAnalytics` (disabled). The resolved emitter is held on `ConfigService` (`config.analytics`, `public readonly`) **outside** the swappable `SodaxConfig`, exactly like `config.logger`.
- **Emit gating — the key to "no needless invocations".** `emit(feature, action, phase, build?, level?)` takes a **lazy thunk** `build` invoked only when the event passes the enabled / per-feature / level gate, so payloads are never constructed when analytics is off, off for that feature, or above the configured detail level. Disabled = one boolean check, nothing else. A throwing sink is swallowed (fire-and-forget) — analytics never breaks a feature flow.
- **`trackResult` is the canonical instrumentation pattern.** Action methods don't scatter `emit` calls at every return point (most have several). Instead each public action method is a one-line wrapper that delegates to a private `*Impl` holding the original body: `return this.config.analytics.trackResult('<feature>', '<action>', () => this.<action>Impl(_params), { start?, success?, failure? })`. `trackResult` emits `start`, runs the impl, then `success` / `failure` from the returned `Result` (and `failure` + rethrow if it throws), returning the `Result` unchanged — so one call at the boundary covers every internal return. The body stays untouched in the `*Impl`. (`SwapService.swap` keeps the body inline inside the `trackResult` arrow instead of an `*Impl` — equivalent; it also passes rich `start`/`success` payloads as the reference example.)
- **Coverage — all 8 features.** Every user-action flow is instrumented (action name mirrors `context.action`): swap → `swap`; moneyMarket → `supply`/`borrow`/`withdraw`/`repay`; bridge → `bridge`; staking → `stake`/`unstake`/`instantUnstake`/`claim`/`cancelUnstake`; migration → `migratebnUSD`/`migrateIcxToSoda`/`revertMigrateSodaToIcx`/`migrateBaln`; dex (`ConcentratedLiquidityService`) → `supplyLiquidity`/`increaseLiquidity`/`decreaseLiquidity`/`claimRewards`; partner (`PartnerFeeClaimService`) → `setSwapPreference`/`swap`; recovery → `withdrawHubAsset`. Most carry only the lifecycle (feature + action + phase) for now; per-action `start`/`success` payloads (like swap's) can be enriched incrementally via each `trackResult` call's `data` builders.
