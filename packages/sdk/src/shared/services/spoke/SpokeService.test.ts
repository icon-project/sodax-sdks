...
// =========================================================================
// 3. srcChainKey / token.chainKey divergence guard
// =========================================================================

describe('SpokeService balance router — chain-key guard', () => {
  // The readers pick their RPC from `srcChainKey` and the identifier from `token.address`, so a
  // foreign-chain token reads the wrong chain. On EVM that surfaces as a failed call the router
  // would otherwise report as a zero balance, hence: reject before dispatching.
  const foreignToken = firstToken(ChainKeys.BASE_MAINNET);

  it('getWalletBalance rejects a token from another chain before dispatching', async () => {
    const spy = vi.spyOn(spoke.evm, 'getWalletBalance');

    const result = await spoke.getWalletBalance({ srcChainKey: ARB, srcAddress: SRC, token: foreignToken });

    expect(result.ok).toBe(false);
    expect(!result.ok && (result.error as Error).message).toMatch(/belongs to chain/);
    expect(spy).not.toHaveBeenCalled();
  });

  it('getWalletBalances rejects the whole batch when any token diverges', async () => {
    const spy = vi.spyOn(spoke.evm, 'getWalletBalances');

    const result = await spoke.getWalletBalances({
      srcChainKey: ARB,
      srcAddress: SRC,
      tokens: [firstToken(ARB), foreignToken],
    });

    // Dropping just the offender would resurface at the call site as `?? 0n` — the silent wrong
    // balance the guard exists to prevent — so the whole batch must fail.
    expect(result.ok).toBe(false);
    expect(spy).not.toHaveBeenCalled();
  });

  it('guards the hub short-circuit too', async () => {
    const spy = vi.spyOn(spoke.sonic, 'getWalletBalances');
