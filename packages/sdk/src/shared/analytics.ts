 ...
 * gate. When analytics is disabled, off for a feature, or above the configured detail level, the
 * `build` thunk is never invoked, so event payloads are never constructed on hot feature paths. This
 * is what keeps feature code free of analytics overhead unless a consumer explicitly opts in.
 */
export interface ResolvedAnalytics {
  /**
   * Whether an event for `feature` at `level` would be emitted. Call sites rarely need this directly
   * — {@link emit} checks it internally — but it is exposed for the rare case where even deciding
   * what to emit is itself costly.
   */
  isEnabled(feature: SodaxFeature, level?: AnalyticsDetailLevel): boolean;
  /**
   * Emit one user-action event. `build` produces the optional structured payload and is invoked
   * lazily — only when the event passes the enabled/feature/level gate — so disabled analytics costs
   * nothing but a boolean check at the call site. A faulty consumer sink can never break a feature
   * flow: delivery is wrapped and fire-and-forget.
   */
  emit(
...
export function resolveAnalytics(option: AnalyticsOption | undefined): ResolvedAnalytics {
  ...
  const { sink, level: configuredLevel = 'basic', features } = option;
  const maxRank = LEVEL_RANK[configuredLevel];

  const isEnabled = (feature: SodaxFeature, level: AnalyticsDetailLevel = 'basic'): boolean => {
    if (LEVEL_RANK[level] > maxRank) return false; // event is more detailed than the configured level
    return features?.[feature] !== false; // per-feature override; omitted features default on
  };

  const emit: ResolvedAnalytics['emit'] = (feature, action, phase, build, level = 'basic') => {
    if (!isEnabled(feature, level)) return;
    try {
      sink.track({ feature, action, phase, level, data: build?.() });
    } catch {
      // Analytics is fire-and-forget — a throwing consumer sink must never break a feature flow.
    }
  };
