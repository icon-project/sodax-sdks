...
export class SwapsApiService implements ResultifiedSwapsApiV2 {
  ...
  constructor(config: SwapsApiConfig, logger: SodaxLogger = consoleLogger, options: SwapsApiServiceOptions = {}) {
    this.config = config;
    // Explicit headers may override the `apiKey` convenience option (same rule as SponsoringApiService),
    // whatever casing they use — see `mergeHeaders`.
    ...
    try {
      ...
      if (error instanceof SwapsApiError) {
        ...
        if (isAuthStatus(error.context.status)) {
          this.logger.warn(
            `[SwapsApiService] ${endpoint} was rejected by the swaps API key guard (${error.context.status}). Configure a valid partner API key — new Sodax({ apiKey }) or new Sodax({ swaps: { apiKey } }) — see docs/CONFIGURE_SDK.md.`,
          );
        }
      }
