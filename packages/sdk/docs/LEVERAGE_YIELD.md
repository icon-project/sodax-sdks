...
Takes an optional second argument, a `RequestOverrideConfig`. Its `apiKey` reaches every keyed leg —
the submit-tx record read, the solver read, and the durable-intent read a solver `NOT_FOUND` falls
