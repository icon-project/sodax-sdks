...
It mirrors `IBridgeApiV2` (from `@sodax/types`) one method per endpoint (7 total). Every method:
...
- **No backend bridgeable-amount endpoint.** Use `sodax.bridge.getBridgeableAmount(...)` +
  `sodax.bridge.isBridgeable({ from, to })`. Only the token *list* is backend-served (`getTokens`).
