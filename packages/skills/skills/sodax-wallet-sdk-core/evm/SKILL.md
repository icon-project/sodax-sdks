...
Granular skill for `EvmWalletProvider` — the low-level EVM wallet for backend / Node / non-React flows. One class covers all 12 SODAX EVM chains — the Sonic hub plus 11 EVM spoke chains — via `getEvmViemChain()`. Source-of-truth reference lives in the parent broad skill's knowledge tree; this file is the focused workflow only.
...
3. **Which chain?** One `EvmWalletProvider` covers all 12 EVM chains (the Sonic hub plus 11 EVM spokes) via `chainId: EvmChainKey` (`ChainKeys.SONIC_MAINNET`, …).
