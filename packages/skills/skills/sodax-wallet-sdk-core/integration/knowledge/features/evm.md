...
| `sendTransaction` | `(txData: EvmRawTransaction, options?: EvmSendTransactionPolicy & EvmSendTransactionOptions) => Promise<Hash>` | viem `Hash` | `defaults.sendTransaction` |
...
- **Chain binding is opt-in.** Pass `options.expectedChainId` (from `@sodax/types` `EvmSendTransactionOptions`) and the provider refuses to send when the wallet's active chain differs — otherwise the tx is broadcast on whatever chain the wallet is connected to. Pass it whenever the calldata was built for one specific chain (the SDK's EVM spoke/hub services already do); take the value from the viem chain config, e.g. `getEvmViemChain(chainKey).id`.
- **`expectedChainId` compares as a plain `number` and fails closed.** The provider reads the wallet's actual chain via `eth_chainId`, which viem normalises from hex to `number` before the strict `!==` compare — there is no format mismatch to worry about. Every degenerate wallet response (non-hex, empty hex, NaN) ends in a thrown error and the tx is NOT sent; a mismatch throws `"wallet is connected to chain X but the transaction targets chain Y"` — catch it to prompt a network switch.
