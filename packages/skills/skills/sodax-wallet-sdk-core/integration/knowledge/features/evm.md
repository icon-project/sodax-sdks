...
- **Chain binding is opt-in.** Pass `options.expectedChainId` (`getEvmViemChain(chainKey).id`) and the provider refuses to send when the wallet's active chain differs. The SDK passes it on spoke/hub deposits and messages, signed ERC-20 approvals (reset leg included), and the hub-scoped vault, partner and BALN sends; `Permit2Service` and `Erc4626Service` do not yet, because no chain key reaches those signatures.
