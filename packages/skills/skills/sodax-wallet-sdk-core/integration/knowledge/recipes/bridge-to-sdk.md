...
For SDK-level recipes (initialise the `Sodax` facade, configure hub provider, raw-tx flows, error handling) load the `sodax-sdk` (integration mode) skill (sibling under `@sodax/skills`).
