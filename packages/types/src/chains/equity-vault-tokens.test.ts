...
// assets on every chain that represents them, deliberately absent from the money market. A
// `SodaTokens` entry is not evidence of money-market support, so this matrix pins both halves.
const MULTI_SPOKE_EQUITY_SYMBOLS = [
  'SPCX',
  'NVDA',
  'GME',
  'MSTR',
...
const EQUITY_SYMBOLS = [...MULTI_SPOKE_EQUITY_SYMBOLS, ...ROBINHOOD_ONLY_EQUITY_SYMBOLS] as const;
...
const ROBINHOOD_ONLY_EQUITY_CHAINS = [ChainKeys.SONIC_MAINNET, ChainKeys.ROBINHOOD_MAINNET] as const;

type EquityRegistry = Partial<Record<EquitySymbol, XToken>>;

const chainsOf = (symbol: EquitySymbol): readonly EquityChain[] =>
  (ROBINHOOD_ONLY_EQUITY_SYMBOLS as readonly EquitySymbol[]).includes(symbol)
    ? ROBINHOOD_ONLY_EQUITY_CHAINS
    : EQUITY_CHAINS;

const representation = (chainKey: EquityChain, symbol: EquitySymbol): XToken => {
  const token = (EQUITY_REGISTRIES[chainKey] as EquityRegistry)[symbol];
  if (!token) throw new Error(`${symbol} has no ${chainKey} representation`);
  return token;
};
...
describe('equity vaults are registered on every chain that represents them', () => {
  for (const symbol of EQUITY_SYMBOLS) {
    ...
    for (const chainKey of chainsOf(symbol)) {
      it(`${symbol}: is in the ${chainKey} token registry, keyed by its own symbol`, () => {
        ...
        expect((spokeChainConfig[chainKey].supportedTokens as EquityRegistry)[symbol]).toBe(token);
      });
...
describe('equity vaults are production swap tokens on every chain that represents them', () => {
  for (const symbol of EQUITY_SYMBOLS) {
    for (const chainKey of chainsOf(symbol)) {
...
describe('equity vaults are not money-market assets', () => {
  const equityAddresses = new Set(
    EQUITY_SYMBOLS.flatMap(symbol =>
      chainsOf(symbol).map(chainKey => representation(chainKey, symbol).address.toLowerCase()),
    ),
  );

  it('has the lists it claims to check — otherwise every assertion below is vacuous', () => {
    expect(equityAddresses.size).toBe(EQUITY_SYMBOLS.reduce((count, symbol) => count + chainsOf(symbol).length, 0));
