...
**RECOMMENDED**: run [`./scripts/bump-versions.sh`](../scripts/bump-versions.sh) from the repo root — it prompts for the new version, validates the format, updates every published `package.json`, and increments `CONFIG_VERSION` in one go.
...
- [ ] 8. Cut a **single unified release tag** — this publishes ALL publishable packages in one workflow run:
  ...
  - [ ] 8.7 The [sdks-publish.yml](../.github/workflows/sdks-publish.yml) workflow will validate that every listed `package.json` version matches the tag, then publish in topological order (see the `PACKAGES` array in the workflow): `types` → `libs` → `swaps-api` → `bridge-api` → `wallet-sdk-core` → `sdk` → `wallet-sdk-react` → `dapp-kit` → `skills`. If validation fails, fix the mismatched `package.json`, bump to the next patch (e.g. `rc.1` → `rc.2`), and re-tag — npm rejects republishing the same version.
