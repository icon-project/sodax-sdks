...
- **Mechanical enforcement via [_mutationContract.test.ts](src/hooks/_mutationContract.test.ts).** 43 hooks × 6 assertions (258 contract tests) run on every `pnpm test` and lock the canonical shape: `useSafeMutation` not `useMutation`, default `mutationKey` before the spread, `mutationFn` after, `unwrapResult` translation, feature-prefix queryKey rule. Adding a new mutation hook requires registering it in the manifest. → see *Adding a New Hook*.
...
~102 hooks (43 mutations + ~55 queries + utilities) organized by feature domain in `src/hooks/`:
...
Every mutation hook returns `SafeUseMutationResult` (extends `UseMutationResult` with `mutateAsyncSafe`). The 43-hook contract is enforced by [_mutationContract.test.ts](src/hooks/_mutationContract.test.ts).
