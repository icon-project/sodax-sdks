...
description: "Mainnet and testnet relay endpoints, chain IDs, actions, and SDK integration for the SODAX intent relay service."
---

<Note>
**Error handling conventions:** Relay-layer failures emit one of two stable strings on `error.message`: `'SUBMIT_TX_FAILED'` or `'RELAY_TIMEOUT'`, also exported as `RELAY_ERROR_CODES` from `@sodax/sdk`. Modules other than swap propagate these errors raw. The **swap module** wraps them into `SodaxError<SwapErrorCode>` with `context.relayCode` (see [SWAPS.md](https://github.com/icon-project/sodax-document/tree/main/developers/packages/sdk/docs/SWAPS.md) Error Handling).
</Note>
