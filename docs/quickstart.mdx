...
<Steps>
  ...
  <Step title="Poll until settled">
    Use `sodax.swaps.getStatus` until the intent is `SOLVED` or `FAILED`. `NOT_FOUND` is not terminal — the solver can answer it for the first few polls after creation, so retry before giving up (the full sample stops after three consecutive ones). Full polling sample: [How to make a swap](/developers/packages/sdk/docs/HOW_TO_MAKE_A_SWAP).
