...
<Note>
  Five minutes is how long it takes to get this code running. How long a submitted swap takes to
  settle is a separate question — see the last step, **Poll until settled**.
</Note>
  ...
  <Step title="Poll until settled">
    ...
    for (let attempt = 0; attempt < 60; attempt++) {
      ...
      if (status.ok) {
        ...
        // creation, so retry before giving up.
        if (status.value.status === SolverIntentStatusCode.NOT_FOUND && ++notFoundCount >= 3) {
          throw new Error('Intent not found after 3 polls');
        }
      }
    ...
    **Settlement time is separate from the five minutes above.** This loop gives up after 5 minutes
    (60 polls × 5s) — tune that budget to your needs. How long a fill actually takes depends on
    solver and relay conditions rather than on your code, so treat `SOLVED` as the only success
