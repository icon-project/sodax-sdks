...
<Steps>
  ...
  <Step title="Initialize">
    ```typescript
    import { Sodax, ChainKeys, type CreateIntentParams, type Hex } from '@sodax/sdk';
  ...
  <Step title="Quote">
    ...
    // Instance configuration includes constructor overrides.
    const inputToken = sodax.config.spokeChainConfig[srcChainKey].nativeToken; // ETH on Arbitrum
    const outputToken = sodax.config.spokeChainConfig[dstChainKey].nativeToken; // BNB on BNB Chain
