---
title: "Intents"
description: "How SODAX coordinates cross-chain trading: a central hub settles intents while solvers fill them from any spoke chain."
---
...
<Steps>
  <Step title="Create">User creates intent on hub chain.</Step>
  <Step title="Lock">Solver sees intent and locks funds on spoke chain.</Step>
  <Step title="Notify">Spoke chain notifies hub of the fill attempt.</Step>
  <Step title="Settle">Hub validates and settles the trade.</Step>
  <Step title="Release">Hub notifies spoke to release funds to user.</Step>
  <Step title="Pay">Solver receives payment on hub chain.</Step>
</Steps>
...
<Note>
This section is implemented by each spoke chain and therefore will not be exactly the same as the EVM implementation.
</Note>
...
- `fillID`: Unique identifier generated by the solver, used to track the fill across chains. Must be unique per intent-solver combination and prevents double-fills.
- `intentHash`: The keccak256 hash of the original intent from the hub, stored in bytes format for cross-chain compatibility. Links the fill to the original intent.
- `solver`: Address of the solving entity in bytes format. Receives payment on the hub chain upon successful fill. Cross-chain format allows for different address formats across chains.
- `receiver`: Destination address for the filled tokens. Matches the `dstAddress` from the original intent, stored in bytes for cross-chain compatibility.
- `token`: Address of the token being transferred, stored in bytes format. Can be the zero address (in bytes) for native currency.
...
<Warning>
Native tokens can only be used on the hub chain. Cross-chain transfers of native tokens are not supported. When using native tokens, the `dstChain` must be set to the hub chain ID.
</Warning>
...
The system uses the `AssetManager` contract to maintain mappings between tokens across different chains. Each token has a home chain (where it originates), an address on each chain where it exists, and a relationship with the hub chain representation.
...
1. **To Hub Chain** (`dstChainID == HUB_CHAIN_ID`): the token is transferred directly to the user.
2. **To Non-Home Chain** (`dstChainID != chainID`): the token is transferred to a wallet abstraction on the hub chain for that specific user.
3. **To Home Chain** (`dstChainID == chainID`): the token is transferred directly to the user via the AssetManager.

### Token Flow Example

For a cross-chain intent where the input token is native to Chain A, the output token is native to Chain B, and the intent is created and settled on the hub chain, the flow is:
...
While all intents created need to be between tokens represented on the hub chain, when filling externally the solver must fill the correct token representation on the spoke chain, which can be queried from the asset manager.

## Fee System

### Overview

The Intents system supports a flexible fee mechanism that allows partner fees to be collected during intent execution. Fees are specified in the intent's data field and are handled automatically during the fill process.
...
- For full fills, the fee is sent to the fee receiver and the remaining amount is sent to the solver.
- For partial fills, the fee is proportionally split: the filled portion's fee is sent to the fee receiver, and the remaining fee is returned to the creator upon cancellation.

**During Cancellation:**

- If the intent is cancelled before any fills, the full fee is returned to the creator.
- If the intent is partially filled, the filled portion's fee is sent to the fee receiver and the remaining fee is returned to the creator.
