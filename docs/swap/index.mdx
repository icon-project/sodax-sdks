...
Start with [Your first swap](/quickstart) for a complete SDK flow from setup to settlement.
For an HTTP integration, follow the [create → submit-tx → poll walkthrough](/developers/http-api/swaps#bot-flow-create-submit-tx-poll).

## Integration reference

<CardGroup cols={2}>
  <Card title="Embeddable widget" icon="window-maximize" href="/swap/widget">
    A hosted swap in one `<iframe>`. Configure and brand it in the browser, no SDK to install.
  </Card>
  ...
  <Card title="SDK method reference" icon="code" href="/developers/packages/foundation/sdk/functional-modules/swaps">
