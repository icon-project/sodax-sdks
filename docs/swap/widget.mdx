...
The listener is optional; the `<iframe>` alone works.
...
<script>
  (() => {
    ...
    window.addEventListener('message', event => {
      ...
      if (data.type === 'sodax:resize' && typeof data.height === 'number' && Number.isFinite(data.height)) {
...
export function SodaxSwapWidget({
  src = 'https://widget.sodax.com/?embed=1',
  height = 760,
  onSwapStatus,
}: SodaxSwapWidgetProps) {
  ...
  useEffect(() => {
    const onMessage = (event: MessageEvent<unknown>) => {
      ...
      const data = event.data as { type?: string; status?: SwapStatus; height?: number };
      if (data?.type === 'sodax:swap' && data.status) onSwapStatus?.(data.status);
      if (data?.type === 'sodax:resize' && typeof data.height === 'number') {
        element.style.height = `${Math.max(760, Math.min(1600, data.height))}px`;
      }
    };
