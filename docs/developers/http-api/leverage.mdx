...
`GET /submit-tx/status` echoes the queued row's discriminator as `data.operation` — `leverage_deposit` / `leverage_withdraw` here, `swap` for a plain swap — so one poller can tell the three apart.
