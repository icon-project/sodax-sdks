...
<Note>
**Preview.** Leverage-yield HTTP routes are available on canary (`https://canary-api.sodax.com`) and may not be enabled on production edge routing yet. Prefer the [SDK leverage yield module](/developers/packages/foundation/sdk/functional-modules/leverage_yield) for product integrations until production routing is confirmed.
</Note>
