...
Send `x-api-key` from your server, never from a browser: a key in front-end code is public. [API key good practices](/developers/how-to/api-key-good-practices) shows how to proxy browser traffic through your own backend.

## Route map

| Prefix | Purpose | Auth |
|--------|---------|------|
| `/v1/swaps/*` | Swaps API — quote, build intent, submit-tx, status | No key enforced yet (rate-limited writes) — see [API keys](/developers/how-to/api-keys) |
