---
title: "Setup Wallet Providers"
description: "How to supply wallet providers to the Sodax SDK — supported interfaces, wallet-sdk-core implementations, per-chain config, React integration, and custom providers."
---
...
<Note>
`rpcUrl` is typed as `` `http${string}` `` (template literal), not a bare `string`. EVM and Injective `rpcUrl` fields have the same constraint. If you pass a `string` from an environment variable, either narrow it explicitly (e.g. `process.env.RPC_URL as \`http${string}\`\`) or validate at the boundary.
</Note>
