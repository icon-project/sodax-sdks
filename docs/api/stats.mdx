...
Use these unauthenticated `GET`s to power dashboards, marketing sites, partner portals, or analytics jobs. There is **no single “TVL” endpoint** — compose metrics from the surfaces below.
...
Returns fee and volume rollups for a fee-receiver address (useful for partner dashboards). Pair with the [Monetize SDK](https://docs.sodax.com/developers/how-to/monetize_sdk) for on-chain fee claim flows.
...
| `GET /v1/be/oracle/markets` | Candle-backed symbol list for price UIs — see [Oracle](/api/oracle) |
