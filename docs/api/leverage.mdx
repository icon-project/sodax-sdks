...
- **Deposit** can carry `partnerFee` (same shapes as swaps: fixed `amount` or `percentage` bps).  
- **Withdraw** currently **cannot** carry a partner fee (SDK constraint on withdraw params). Partners earn on the way in only for vault flows.
...
| Partner fee | either direction | deposit only |
