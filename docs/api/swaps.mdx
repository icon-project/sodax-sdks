...
| **TypeScript client** | `@sodax/swaps-api` / `sodax.api.swaps` |
...
On success, `result.fillTxHash` is the destination fill when available (optional — journal-confirmed fills may omit it). `result.dstIntentTxHash` / `intent_hash` / `packetData` appear depending on path.
