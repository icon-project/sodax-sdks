...
- Empty ranges return **`200` with `"candles": []`** (never `404`). Render gaps gracefully.
...
Quotes for swaps should go through the [Swaps API](/api/swaps) or the SDK. The solver oracle is for display, sanity checks, and tooling — not a substitute for a firm quote.
...
| Unknown symbol or interval | `400` |
| Range wider than 5000 candles | `400` |
| Empty time range | `200` + empty `candles` array |
