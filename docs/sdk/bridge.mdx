---
title: "Bridge"
description: "Cross-chain token transfers across the SODAX hub-and-spoke architecture, including allowance, approval, bridgeable-amount lookups, and error handling."
---
...
If you were on the previous CODE-on-`error.message` pattern (or the older `BridgeError<Code>` typed shape that the published docs at [/sdk/bridge#error-handling](/sdk/bridge#error-handling) document), here are the mappings:
...
**Check allowance** using `isAllowanceValid()`
</Step>
<Step title="Approve tokens">
**Approve tokens** using `approve()` if needed
</Step>
<Step title="For Stellar destination chains">
**For Stellar destination chains**: check and establish trustlines (see [Stellar Trustline Requirements](https://github.com/icon-project/sodax-sdks/blob/main/packages/sdk/docs/STELLAR_TRUSTLINE.md#bridge))
</Step>
<Step title="Execute bridge">
**Execute bridge** using `bridge()` for the full lifecycle, or `createBridgeIntent()` for manual relay control
</Step>
<Step title="Monitor progress">
**Monitor progress** using the returned transaction hashes
</Step>
</Steps>
