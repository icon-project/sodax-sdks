---
title: "Money Market"
description: "Supply, borrow, withdraw, and repay across chains through the SODAX cross-chain money market, plus reserve data retrieval, formatting, and error handling."
---
...
If you were on the previous CODE-string-on-`error.message` pattern (or the older `MoneyMarketError<Code>` typed shape that the public docs at [/sdk/money-market#error-handling](/sdk/money-market#error-handling) document), here are the mappings:
...
<Step title="Build Formatting Requests">
```typescript
