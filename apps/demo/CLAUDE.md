...
- **Per-intent partner fees must be reflected in the quote.** Leverage-yield deposits charge `DEPOSIT_PARTNER_FEE` (1%, `pages/leverage-yield/page.tsx`) via the swap layer's per-intent override; `createIntent()` deducts the fee from `inputAmount` before the swap, so the quote is requested on the post-fee amount (`adjustAmountByFee(amount, fee, 'exact_input')`). If a fee and its quote drift apart, `minOutputAmount` becomes unfillable and intents never settle.
