...
- What flows: every feature's user-action methods emit `start` / `success` / `failure` (swap, money-market supply/borrow/withdraw/repay, bridge, staking, migration, dex liquidity, partner, recovery) — exercise any feature page and watch the console / a `sodax:analytics` listener. No demo change is needed as SDK emit-sites are enriched.
