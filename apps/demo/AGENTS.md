...
- **Swaps API page drives the original wire client directly.** `pages/swaps-api` + `components/swaps-api` call `@sodax/swaps-api` through a `SwapsApi` instance from `useSwapsApiClient()` (`components/swaps-api/lib/swapsApi.ts`) using plain React Query — deliberately not the dapp-kit `useSwapsApi*` hooks — while wallet/signing and chain prerequisites (trustline, NEAR storage, Bitcoin/Bound) stay on dapp-kit / wallet-sdk-react hooks. Like `apps/swap-api-example`, it exists to exercise the standalone client; don't "migrate" it back to the hooks. **The client must stay on the demo's effective swaps config** — `useSwapsApiClient` builds it from `effectiveSwapsApiBaseUrl` / `effectiveSodaxApiKey`, the same resolvers `providers.tsx` feeds the SDK, because the page's own `OrderStatusPanel` polls submit-tx status through `sodax.api.swaps`: a client with its own base URL submits to one deployment and polls another, so every order sits pending forever, and one with no `apiKey` fails key-gated routes the demo has credentials for. Approval is the one broadcast step dapp-kit covers — `handleApprove` hands the `{ tx, resetTx? }` plan to `runApprovalPlan`, which sequences the reset before the approve and checks each receipt's execution status; don't re-hand-roll that wait. **Two policies the hooks used to supply are now the page's own.** Its React Query keys are prefixed `['demo', 'swapsApi', …]` so dapp-kit's prefix invalidations (`useSwapsApiApproveAndBroadcast` clears `['swapsApi','allowance']`) can never reach a cache entry this page filled with a different client — don't drop the `demo` segment back. And `retry` is `retryUnlessSwapsApiAuthFailure` (`lib/swapsApi.ts`), not dapp-kit's `retryUnlessAuthFailure`: the latter is gated on `isAuthFailure` → `isSodaxError`, and the wire client's `SwapsApiError` only becomes a `SodaxError` by passing through `SwapsApiService`, so on this page it would never match and a rejected API key would retry three times.
...
- **An API `baseURL` is the gateway root, never a service path.** `api.baseURL` (and every per-call `RequestOverrideConfig.baseURL`) is the origin plus the deployment's version prefix — `DEFAULT_API_BASE_URL` in `@sodax/types`, never a copied literal. The SDK appends each service's own path below it (`/be`, `/swaps`, `/bridge`, `/leverage-yield`, `/sponsorships/stellar`), so a segment in the base URL relocates the sibling services too. `providers.tsx` therefore sets `baseApiConfig` **only** when the settings modal overrides the gateway, and sets `swapsApiConfig` to the already-resolved `effectiveSwapsApiBaseUrl(settings)` (`settings.swapsApiBaseUrl ?? VITE_SWAPS_API_BASE_URL ?? settings.apiBaseUrl ?? DEFAULT_API_BASE_URL`, mirroring `layerConfigs` precedence) so the Swaps API page's direct client can be built from the same value; Bridge SDK API calls follow `baseApiConfig`; the Bridge API showcase passes a per-call base URL resolved from `settings.bridgeApiBaseUrl ?? VITE_BRIDGE_API_BASE_URL ?? canary`; the Leverage Yield API showcase passes its own per-call base URL from `LEVERAGE_YIELD_API_CONFIG` (`components/leverage-yield-api/lib/config.ts`), resolved from `VITE_LEVERAGE_YIELD_API_BASE_URL` alone — no settings field, and anything without an `http(s)://` scheme falls back to `DEFAULT_API_BASE_URL`; and `sponsoringApiConfig` comes from its `VITE_*` vars alone (the settings modal has no sponsoring field). With no env vars and no overrides, SDK services run against the packaged production gateway while the Bridge API showcase defaults to canary. A `baseURL` that still ends in `/be` is trimmed with a console warning.
