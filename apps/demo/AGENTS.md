...
- **Swaps API page drives the original wire client directly.** `pages/swaps-api` + `components/swaps-api` call `@sodax/swaps-api` through a shared `SwapsApi` instance (`components/swaps-api/lib/swapsApi.ts`; canary `/v1` by default, override with `VITE_SWAPS_API_BASE_URL`) using plain React Query — deliberately not the dapp-kit `useSwapsApi*` hooks — while wallet/signing and chain prerequisites (trustline, NEAR storage, Bitcoin/Bound) stay on dapp-kit / wallet-sdk-react hooks. Like `apps/swap-api-example`, it exists to exercise the standalone client; don't "migrate" it back to the hooks.
