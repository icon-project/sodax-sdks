...
Regression coverage for CJS consumers: when `@sodax/sdk` is consumed from CommonJS, transitive ESM-only dependencies must not surface `ERR_PACKAGE_PATH_NOT_EXPORTED` to users.
