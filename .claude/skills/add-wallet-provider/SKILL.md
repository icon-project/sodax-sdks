...
## Docs
Docs Drift CI fails unless a related publishable site surface changed with `src/`. Update `packages/wallet-sdk-react/docs/` (Case A) or `packages/wallet-sdk-core/README.md` (Case B). If `packages/types/src` changed too, note that neither of those covers it — update a mapped `packages/sdk/docs/` page such as `WALLET_PROVIDERS.md`, since the `types` README passes the gate but does not publish. JSDoc is not enough. If you add a brand-new published page, add it to `scripts/gitbook-sync-map.json` and give it a nav entry in `docs/docs.json`, or it is live but absent from the sidebar and search; a page not ready to go live goes on the map's `unpublished` list instead. `packages/skills` is partner-agent docs (how integrators connect/sign) — update it when that public surface changed, then run `pnpm check:ai`; it does not satisfy Docs Drift. An unrelated mapped file (for example `packages/skills/README.md`) does not satisfy a wallet-package source change.
