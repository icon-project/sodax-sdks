...
- Docs Drift (job **Docs ship with code**): each package whose `src/` changed needs a *related* mapped doc (`packages/<pkg>/…` in `scripts/gitbook-sync-map.json`, a mapped `packages/sdk/docs/` page, or a mapped root-level `docs/` guide whose `pkgs` array lists the package), the package README, or `packages/<pkg>/docs/` (non-sdk). JSDoc, unmirrored sdk/docs pages, `packages/skills`, an unrelated mapped file, and deleting a README or docs file do not count; moving source out of `src/` still counts as a source change. A new or renamed `packages/sdk/docs/` page must be in the map even without an `src/` change, and every mapped src must exist and be a `.md`/`.mdx` page. The `docs-not-needed` label is the only skip.
