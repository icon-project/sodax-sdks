...
- **Mirrored docs** — Docs Drift CI requires a related publishable site signal for every package whose `src/` changed. Update `packages/sdk/docs/WALLET_PROVIDERS.md` (and any other mapped feature doc the chain enables) plus the affected package README / `packages/wallet-sdk-react/docs/`. JSDoc is not enough. A new `packages/sdk/docs/` page must be added to `scripts/gitbook-sync-map.json` (sodax-document copies the map; add the sidebar entry on the docs-sync PR).
