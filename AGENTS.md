...
`pnpm test:e2e` does **not** run in CI: its job in `ci.yml` is disabled (`if: false`) because it hits live mainnet services and fails on state no branch controls (a solver that dropped an intent from memory, an unindexed relay tx, on-chain token/vault drift). Run it locally when you touch a flow it covers — green CI does not mean the e2e suite passed.
