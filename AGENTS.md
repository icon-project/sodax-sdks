...
| `apps/playground` | Embeddable SODAX swap widget and visual builder: quotes, wallet-backed execution, activity tracking and integration exports | [`apps/playground/AGENTS.md`](apps/playground/AGENTS.md) |
...
- Biome is the only formatter/linter — do not add ESLint or Prettier config. Build order matters: packages build before apps.
...
- **Flag breaking changes to published packages.** Treat anything a consumer of `@sodax/types` / `@sodax/sdk` / `@sodax/dapp-kit` compiles or branches against as a contract: a removed or renamed export, a changed signature or return type, changed runtime behaviour of an exported helper, and **widening a shared enum or union** — adding a member breaks an exhaustive `Record<Enum, T>`, a `satisfies` map, or a `switch` with a `never` default in consumer code. Additive at the value level is not the same as non-breaking at the type level, and the gates here will not tell you: no package in this repo exercises those patterns, so a real consumer break stays green through `build`, `checkTs` and `test`. Decide it while planning, and state it in the PR description under a **Breaking change** heading with the consumer code that breaks and the migration — never leave it for a reviewer to discover.
...
**Definition of done:** scoped diff · behavior verified against `src/` · relevant `test`/`checkTs`/`lint`/`check:ai` green · mapped docs when package `src/` changed · `packages/skills` updated when public behavior changed · breaking changes to published packages called out in the PR description · no unrelated refactor.
...
A `Detect changed paths` job decides whether `Build and Test` runs: a pull request touching only `docs/` skips it, and `Docs site` still gates the change. It is a job-level `if:` rather than a workflow `paths-ignore` on purpose — a path-filtered workflow never reports, so a required check would sit pending forever. The `No private hosts` job (`private-hosts`) sits outside that gate on purpose: it greps the tree for the private `iconblockchain` vendor domain on every pull request — this repo is public, so an endpoint a consumer can reach must be `api.sodax.com` or `canary-api.sodax.com` — and gating it would let exactly the diff it exists to catch skip it. Its script assembles the domain from parts so the workflow file does not match its own pattern, and prose about the guard — this paragraph included — names the domain without its TLD for the same reason.

A `Docs auto-merge` workflow approves and squash-merges a pull request that changes only marketing's docs pages, once `Docs site` is green, so marketing publishes from the Mintlify dashboard without a reviewer. `.github/scripts/classify-docs-pr.sh` decides from an allowlist of the Home, Solutions, Community and Help tabs, and fails closed: the developer reference (`docs/developers/` except `faq`, and `docs/solana/`), `docs.json`, a `generatedFrom` page, an add, delete or rename, or any path outside the allowlist all send the PR to a human. It runs on `pull_request_target` so a PR cannot edit its own gate, and classifies before requesting an App token. Non-marketing changes need no App credentials unless the App is provisioned and a queued auto-merge or active bot approval may need cleanup; read-only PR metadata keeps that cleanup in scope. Approval uses a GitHub App because `GITHUB_TOKEN` reviews do not count toward a required approval. Just before approving it composes a conventional `docs(marketing): …` subject naming the pages changed, sets it as the PR title and passes it to the merge as `--subject`, because the Mintlify editor defaults to a `Draft from <date>` title and an `Updated mintlify pages` commit. That retitle takes the same head pin the approval does, and banks the title it replaced in the description so withdrawal restores it when the PR stops qualifying. Naming the subject per merge is deliberate: `squash_merge_commit_title` and the `Lint PR` single-commit options are left as they are, so no other pull request changes behaviour. Setup and the admin-only steps: [`.github/docs-publishing.md`](.github/docs-publishing.md).
