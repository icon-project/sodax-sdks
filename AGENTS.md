...
- When a generated doc is added, renamed, or removed, update its `scripts/docs-pages-map.json` entry and the `docs.json` nav entry for its `dest` together, then run `pnpm docs:sync-pages`. Gates: `pnpm check:docs-pages` and `pnpm check:docs-nav`.
