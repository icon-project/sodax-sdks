...
A separate `Docs Drift` PR check (`.github/scripts/check-docs-drift.sh`) fails when package runtime source changes without touching any docs surface (`packages/sdk/docs/`, a package `README.md`, `packages/skills`, root `docs/`, or JSDoc in the diff); maintainers bypass it with the `docs-not-needed` label. Downstream publication is pulled by a scheduled sync workflow in `sodax-document`.
