...
- **New mirrored docs must also be in nav.** Adding a page to `scripts/gitbook-sync-map.json` publishes it, but that alone does not make it discoverable. A page under `docs/` needs its matching entry in `docs/docs.json`; a mirrored page elsewhere gets its sidebar entry on the docs-sync side. Without one the page is live but absent from the sidebar and from search. Docs Drift does not check nav.
...
- When a mirrored doc is added, renamed, or removed, update `scripts/gitbook-sync-map.json` — every mapped src is published — and add or remove the matching `docs/docs.json` nav entry for pages under `docs/`.
