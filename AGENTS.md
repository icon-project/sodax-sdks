...
A separate `Docs Drift` PR check (job name **Docs ship with code**, script `.github/scripts/check-docs-drift.sh`) fails when package runtime source changes without a *related* publishable docs signal: a mapped file under that package, a mapped `packages/sdk/docs/` page, the package `README.md`, or `packages/<pkg>/docs/` (non-sdk). JSDoc, unmirrored `packages/sdk/docs/` pages, `packages/skills`, an unrelated mapped file, and deleting a README or docs file do not count. A newly added `packages/sdk/docs/**/*.md` must be added to the map even on a docs-only PR. Maintainers bypass the check with the `docs-not-needed` label. Downstream publication is pulled by a scheduled sync workflow in `sodax-document`.
